...
text: "*All Transaksi Open ✅*\n\n*FiX* Menyediakan Produk & Jasa Dibawah Ini ⬇️"
}), 
contextInfo: {
...
case "fixdila": {
let teksnya = `
こんにちは @${m.sender.split("@")[0]},
私の名前は *${global.botname2}*. です。下のボタンを押してボットを実行します。ジェンドラはディラが大好き
`
await conn.sendMessage(m.chat, {
  ...
  buttons: [
    {
      buttonId: `.dilaa`,
      buttonText: { displayText: 'Dilaa' },
      type: 1
    },
    {
      buttonId: `.jendlaa`,
      buttonText: { displayText: 'Jendlaa' },
      type: 1
    }
  ],
  headerType: 1,
...
await conn.sendMessage(m.chat, {
  ...
  buttons: [
    {
      buttonId: `.jendlaa`,
      buttonText: { displayText: 'Dilaa cayangg' },
      type: 1
    },
    {
    buttonId: 'action',
...
const dirsesi = fs.readdirSync("./fixsession").filter(e => e !== "creds.json")
const dirsampah = fs.readdirSync("./library/database/sampah").filter(e => e !== "A")
for (const i of dirsesi) {
await fs.unlinkSync("./fixsession/" + i)
}
...
var anu = await fetchJson("https://api.FiX.us.kg/api/download/ytmp3?url="+res.url)
...
var anu = await fetchJson("https://api.FiX.us.kg/api/download/ytmp4?url="+res.url)
...
var anu = await fetchJson("https://api.FiX.us.kg/api/download/ytmp3?url="+text)
if (anu.download.audio) {
let urlMp3 = anu.download.audio
...
if (!text.startsWith("https://")) return m.reply("Link Tautan Tidak Valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
var anu = await fetchJson("https://api.FiX.us.kg/api/download/ytmp4?url="+text)
if (anu.download.video) {
let urlMp3 = anu.download.video
...
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://api.FiX.us.kg/api/search/happymod?q=${text}`).then((res) => {
let teks = ""
...
if (!text.startsWith('https://')) return m.reply("Link tautan tidak valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://api.FiX.us.kg/api/download/igdl?url=${text}`).then(async (res) => {
if (!res.status) return m.reply("Error! Result Not Found")
...
if (!text.startsWith('https://')) return m.reply("Link tautan tidak valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://api.FiX.us.kg/api/download/fbdl?url=${text}`).then(async (res) => {
if (!res.status) return m.reply("Error! Result Not Found")
...
if (!text.startsWith('https://')) return m.reply("Link tautan tidak valid")
await conn.sendMessage(m.chat, {react: {text: '🕖', key: m.key}})
await fetchJson(`https://api.FiX.us.kg/api/download/capcut?url=${text}`).then(async (res) => {
if (!res.status) return m.reply("Error! Result Not Found")
...
await conn.sendMessage(m.chat, {react: {text: '🔎', key: m.key}})
const res = await fetchJson(`https://api.FiX.us.kg/api/search/gimage?q=${text}`)
...
await fetchJson("https://api.FiX.us.kg/api/ai/openai-prompt?prompt=Sekarang Kamu Adalah Skyzo-Ai dan selalu gunakan bahasa Indonesia saat menjawab semua pertanyaan&msg=" + talk).then(async (res) => {
...
let { directLink } = await service.uploadFromBinary(fs.readFileSync(media), 'FiXOffc.jpg');
...
await m.reply("Kudeta Grup By FiX Starting 🔥")
...
case "developerbot": case "jendlaa": {
...
var file = await fs.readdirSync("./fixsession")
var anu = await file.filter(i => i !== "creds.json")
for (let t of anu) {
await fs.unlinkSync(`./fixsession/${t}`)
}
